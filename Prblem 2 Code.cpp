...
public:
	...
	int leftHeavy(node* p1);


};
int tree::leftHeavy(node* p1) {
	if (!p1) return -9999;
	if (!p1->l_child) return p1->value;
	int n1, n2;
	n1 = leftHeavy(p1->l_child);
	n2 = leftHeavy(p1->r_child);
	if (n1 < n2) swap(p1->l_child, p1->r_child);
	return n1 + n2 + p1->value;
